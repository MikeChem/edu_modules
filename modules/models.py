...
class Course(models.Model):
    ...
    author = models.ForeignKey(User, on_delete=models.CASCADE, related_name="courses", verbose_name=_("Автор"))
...
class Module(models.Model):
    course = models.ForeignKey(
        Course, on_delete=models.CASCADE, null=True, related_name="modules"  # ← Позволяем временно пустое значение
    )
    title = models.CharField(_("Заголовок модуля"), max_length=255)
    description = models.TextField(_("Описание"), blank=True, null=True)
    author = models.ForeignKey(User, on_delete=models.CASCADE, related_name="modules", verbose_name=_("Автор"))
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Material(models.Model):
    lesson = models.ForeignKey(Lesson, on_delete=models.CASCADE, related_name="materials", verbose_name=_("Урок"))
...
class Test(models.Model):
    lesson = models.OneToOneField(
        Lesson, on_delete=models.CASCADE, related_name="test", verbose_name=_("Тест для урока")
    )
...
class UserProgress(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="progress", verbose_name=_("Пользователь"))
